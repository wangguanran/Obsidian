# SM8550 gpiotest GPIO 全引脚测试脚本需求实现
...
- 移除 green(gpio389)/red(gpio317) LED 测试：389/317 已纳入环回矩阵（389<->407, 317<->310），重复 export 会 busy/覆盖方向，使环回结果不可信；其环回验证由 `gpioAllTest` 段完成
- blue(gpio318) 不在矩阵中，保留 LED 测试（输出 1→0 验证）
- 新增 PM8550 GPIO ADC 测试：`pm8550_gpio2`（<1900000µV）、`pm8550_gpio12`（<1800000µV）电压阈值判断
...
1. 脚本对 `gpio_num1`（out 组）输出 1、`gpio_num2`（in 组）读取环回电平，方向与电平正确则记 `true`
2. 单引脚组（`gpio_single_input*`）输出后回读验证
3. PM8550 ADC：`pm8550_gpio2` 电压 < 1.9V、`pm8550_gpio12` 电压 < 1.8V 判定通过
4. blue LED（gpio318）：输出 1 时亮度 0 验证、输出 0 时亮度 255 验证
5. 全部通过则 `test_result=true`

### 结果检查

```bash
adb shell cat /mnt/vendor/persist/gpiotest/gpio_test_final.ini
# 期望末尾: [final_result] test_result=true
```

> ⚠️ 该补丁在 134 服务器无对应源码树（LA.VENDOR.13.2.9 未部署），无法实机验证，以上为脚本逻辑推演的预期行为。
...
index 79c0414..5d45891 100644
--- a/device/qcom/kalama/AndroidBoard.mk
+++ b/device/qcom/kalama/AndroidBoard.mk
@@ -55,7 +55,7 @@
 #----------------------------------------------------------------------
 #for gpiotest
 include $(CLEAR_VARS)
...
-#pins = "gpio44", "gpio45";
-#gpio偏移 301
- #gpio_num1=(2    4     6    8    10   48   175   180  190   188   106   159   196   198   140       37    60     142   152    161   163        179    186   56    58   64    66    68    70   192   194   121   123         165   167   169   171   173   177   182   187      pm8550ve_gpio4   pm8550ve_gpio6   pm8550ve_gpio8   pmk8550_gpio5  pm8550vs_c_gpio4  pm8550vs_d_gpio3   pm8550vs_g_gpio4   pm8550vs_e_gpio5)
- #gpio_num2=(3    5     7    9    12   49   176   181  191   189   107   160   197   199   141       36    61     72    153    162   164        108    154   57    59   65    67    69    71   193   195   122   124         166   168   170   172   174   178   183   129      pm8550ve_gpio5   pm8550ve_gpio7   pm8550vw_gpio6   pm8550_gpio1   pm8550vs_c_gpio5  pm8550vs_e_gpio4   pm8550ve_gpio3     pm8550vs_e_gpio6)
- gpio_num1=(303   305   307  309  311  349  476   481  491   489   407   460   497   499   441      338   361     443   453    462   464        480    487   357   359  365   367   369   371  493   495   422   424         466   468   470   472   474   478   483   488      284              286              288              255    262     278               271               260                 267)
- gpio_num2=(304   306   308  310  313  350  477   482  492   490   408   461   498   500   442      337   362     373   454    463   465        409    455   358   360  366   368   370   372  494   496   423   425         467   469   471   473   475   479   484   430      285              287              280              289    261     279               266               283                 268)
-#gpio_num3=(   )
-###
-gpio_single_input1=(316)
-gpio_single_input2=(456)
-gpio_single_input3=(315 314 331 332)
-gpio_single_input4=(342  341 426 427 428 429 501  451 452 277 )
...
-		out_pull_dirction=`cat /sys/class/gpio/gpio389/direction`
-		echo $out_pull_dirction
-		out_pull_state=`cat /sys/class/gpio/gpio389/value`
-		echo $out_pull_state
-		
-		if [[ $out_pull_dirction == out ]] && [[ $out_pull_state -eq 1 ]]
-		then
-			echo "gpio389=true" >> /mnt/vendor/persist/gpiotest/gpio_test.ini
-		else
-			echo "gpio389=false" >> /mnt/vendor/persist/gpiotest/gpio_test.ini
-			test_result=0
-		fi
-		echo 389 > /sys/class/gpio/export
-		echo out > /sys/class/gpio/gpio389/direction		
...
-		out_pull_dirction=`cat /sys/class/gpio/gpio317/direction`
-		echo $out_pull_dirction
-		out_pull_state=`cat /sys/class/gpio/gpio317/value`
-		echo $out_pull_state
-		
-		if [[ $out_pull_dirction == out ]] && [[ $out_pull_state -eq 1 ]]
-		then
-			echo "gpio317=true" >> /mnt/vendor/persist/gpiotest/gpio_test.ini
-		else
-			echo "gpio317=false" >> /mnt/vendor/persist/gpiotest/gpio_test.ini
-			test_result=0
-		fi
-		echo 317 > /sys/class/gpio/export
-		echo out > /sys/class/gpio/gpio317/direction		
...
+
+
diff --git a/device/qcom/kalama/init.target.rc b/device/qcom/kalama/init.target.rc
index 65a4875..e41f2a5 100755
--- a/device/qcom/kalama/init.target.rc
+++ b/device/qcom/kalama/init.target.rc
@@ -305,7 +305,7 @@
     disabled
     oneshot
 #for gpiotest
...
+service meig_gpio_test /vendor/bin/sh /vendor/bin/SRM969_gpio.sh gpio_test
     ...
     user root
     group root system 
diff --git a/device/qcom/kalama/kalama.mk b/device/qcom/kalama/kalama.mk
index 720df08..8d5d677 100644
--- a/device/qcom/kalama/kalama.mk
+++ b/device/qcom/kalama/kalama.mk
@@ -254,7 +254,7 @@
 PRODUCT_BRAND := qti
...
- [[01.驱动文档/GPIO/Qualcomm/SM8550-A16/91.源码与补丁索引/patches/197519.patch|197519.patch]]（清理隐私后完整补丁，42.8KB）
