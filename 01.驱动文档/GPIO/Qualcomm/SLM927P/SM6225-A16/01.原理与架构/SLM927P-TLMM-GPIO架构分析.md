# Qualcomm SM6225 (SLM927P/khaje) TLMM GPIO 架构分析

> **版本号：v1.0**

## 平台概述

SLM927P（基于 khaje/SM6225）的 GPIO 由 **TLMM**（Top Level Mode Multiplexer）统一管理：
...
`SLM927P_gpio.sh` 遍历所有 GPIO 引脚做功能验证。测试失败的两类根因：
...
2. `SLM927P_gpio.sh`：补充正确测试逻辑（+5 行），排除特殊功能引脚误判（+5/−27 净调整）

## 引用文件索引

- [[01.驱动文档/GPIO/Qualcomm/SLM927P/SM6225-A16/04.问题案例/SLM927P-GPIO全引脚测试失败修复.md|SLM927P-GPIO全引脚测试失败修复]]（补丁内容）
- `kernel_platform/qcom/proprietary/devicetree/qcom/khaje.dtsi`（远程）
- `device/qcom/bengal_515/SLM927P_gpio.sh`（远程）
