    ...
    if(u1IsLP4Family(p->dram_type))  //already read MR8 for density update
    {
        ...
        switch (p->density)
        {
            ...
            case 0x2:   //8Gb per die  (4Gb per channel),  tRFCab=180
                ...
                if(p->revision_id==0x87 && p->vendor_id==0xff) //rayson 2GB change to 1GB
...
void vApplyProgramSequence(DRAMC_CTX_T *p)
{
    ...
    //TMRS(2386+ 2387) FIFO P/G Failure Prevention for all SS 1znm LP4x "MCP" products
    if((gu2MR5_Value[0] & 0xF) == 0x1)
    {
    ...
    if( apply_cond == TRUE)
    {
        ...
        ProgramCodeInput(p, 3, 9, 0);
        ProgramCodeInput(p, 1, 2, 0);
        ProgramCodeInput(p, 3, 8, 6);
        ProgramCodeInput(p, 3, 8, 7);
        mcSHOW_DBG_MSG(("TMRS 2386 2387: TMRS enter -> 000 -> 390 -> 120 -> 386 -> 387\n"));
        apply_cond = FALSE;
    }
    #endif
