...
static int __android_start_enabled = 0;
static ssize_t android_start_enabled_show(struct device *dev, struct device_attribute
				     *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d\n", __android_start_enabled);
}

static ssize_t android_start_enabled_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int val;
    struct smb2 *chip = dev_get_drvdata(dev);
    struct smb_charger *bcdev = &chip->chg;
	...
	__android_start_enabled = val;
    rc = power_supply_get_property(bcdev->batt_psy,POWER_SUPPLY_PROP_STATUS,&pval);
       if (rc < 0){
        pr_info("[MGQBC] Failed to get ext fg charge type rc=%d\n", rc);
    }else
    {
        pr_err("[MGQBC]Set temp as ext fg charge status: %d \n", pval.intval);
        if( (pval.intval ) == POWER_SUPPLY_STATUS_FULL)
            pr_err("[MGQBC]Set temp as ext fg charge status: %s\n", "charge full" );
        else
            pr_err("[MGQBC]Set temp as ext fg charge status: %s\n", pval.intval == POWER_SUPPLY_STATUS_CHARGING ? "charging":"discharging" );
    }

	pr_err("android start successfully  %d\n", __android_start_enabled );

	return count;
}
static DEVICE_ATTR_RW(android_start_enabled);
...
//ajay
static ssize_t pogo8pin_5v_show(struct device *dev, struct device_attribute
				     *attr, char *buf)
{
        struct smb2 *chip = dev_get_drvdata(dev);
        struct smb_charger *chg = &chip->chg;
	return snprintf(buf, PAGE_SIZE, "%d\n",gpio_get_value(chg->pogo8pin_5v_out));
}

static ssize_t pogo8pin_5v_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int val;
        struct smb2 *chip = dev_get_drvdata(dev);
	...
	gpio_direction_output(chg->pogo8pin_5v_out,!!val);

	return count;
}
static DEVICE_ATTR_RW(pogo8pin_5v);
static ssize_t pogo8pin_remove_count_show(struct device *dev, struct device_attribute
				     *attr, char *buf)
{
	struct smb2 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;

	return snprintf(buf, PAGE_SIZE, "%d\n", chg->pogo8pin_remove_count);
}
static DEVICE_ATTR_RO(pogo8pin_remove_count);
static ssize_t pogo6pin_remove_count_show(struct device *dev, struct device_attribute
				     *attr, char *buf)
{
	struct smb2 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;

	return snprintf(buf, PAGE_SIZE, "%d\n", chg->pogo6pin_remove_count);
}
static DEVICE_ATTR_RO(pogo6pin_remove_count);

static ssize_t pogo8pin_is_insert_show(struct device *dev, struct device_attribute
				     *attr, char *buf)
{
	struct smb2 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;

	return snprintf(buf, PAGE_SIZE, "%d\n", chg->last_pogo8pin_isinsert);
}
static DEVICE_ATTR_RO(pogo8pin_is_insert);
static ssize_t pogo6pin_is_insert_show(struct device *dev, struct device_attribute
				     *attr, char *buf)
{
	struct smb2 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;

	return snprintf(buf, PAGE_SIZE, "%d\n", chg->last_pogo6pin_isinsert);
}
static DEVICE_ATTR_RO(pogo6pin_is_insert);
//<!–– add by meiglink for m100p-497  20231010 begin
static ssize_t pogoadc_value_show(struct device *dev, struct device_attribute
				     *attr, char *buf)
{
	struct smb2 *chip = dev_get_drvdata(dev);
	struct smb_charger *chg = &chip->chg;
	...
	if (!chg->pogo_device_adc_chan || PTR_ERR(chg->pogo_device_adc_chan) == -EPROBE_DEFER){
		chg->pogo_device_adc_chan = iio_channel_get(chg->dev, "pogo_vadc");//pogo_device_adc
	}

	if (IS_ERR(chg->pogo_device_adc_chan)){
		pr_err("err to get pogo_device_adc_chan\n");
	}else{
		iio_read_channel_processed(chg->pogo_device_adc_chan, &val);
		pr_err("get pogo_device_adc_chan  val %d\n", val );
	}

	return snprintf(buf, PAGE_SIZE, "%d\n", val);
}
static DEVICE_ATTR_RO(pogoadc_value);
// add  by meiglink for m100p-497 20231010 end ––>
static struct attribute *smb2_attrs[] = {
	&dev_attr_weak_chg_icl_ua.attr,
	&dev_attr_try_sink_enabled.attr,
	&dev_attr_android_start_enabled.attr,
	&dev_attr_audio_headset_drp_wait_ms.attr,
	&dev_attr_pogo8pin_5v.attr,
	&dev_attr_pogo8pin_remove_count.attr,
	&dev_attr_pogo6pin_remove_count.attr,
	&dev_attr_pogo8pin_is_insert.attr,
	&dev_attr_pogo6pin_is_insert.attr,
        &dev_attr_pogoadc_value.attr,
	NULL,
};
ATTRIBUTE_GROUPS(smb2);

//<!–– add by meiglink for task34189  20230426 begin
static int smb2_disable_typec(struct smb_charger *chg);
static int smb2_configure_typec(struct smb_charger *chg);
bool irq_count = true;
int dcin_det_present=1;
static irqreturn_t pogopin_handler(int irq, void *dev_id);
static void pogodcin_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
							pogo_dcin_work.work);
	int ret = 0;
	chg->pogo6pin_dcin_det_irq = gpio_to_irq(chg->pogo6pin_dcin_det);
	ret = devm_request_threaded_irq(chg->dev, chg->pogo6pin_dcin_det_irq , NULL,
			pogopin_handler,
			IRQF_TRIGGER_LOW | IRQF_ONESHOT,
			"pogo_dcin_irq", chg);
	if (ret) {
		pr_err("Failed pogo_dcin_irq irq=%d request rc = %d\n",
			chg->pogo6pin_dcin_det_irq,  ret);
		return;
	}
	pr_err("[meig xxx] pogodcin_work irq success\n");
}
static void pogopin_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
							pogo_pin_work.work);
	int rc = 0;
	int dcin_det_stat=0;
	dcin_det_stat = gpio_get_value(chg->pogo6pin_dcin_det);
    pr_err("pogopin_work get dcin is %d  %s and will be %s charg\n",  dcin_det_stat, dcin_det_stat?"remove":"insert",dcin_det_stat?"USB":"FLOAT");
	if(dcin_det_present!=dcin_det_stat)
	{
		dcin_det_present=dcin_det_stat;
		if(dcin_det_stat==0)
		{
			rc = smblib_masked_write(chg, TYPE_C_CFG_REG, APSD_START_ON_CC_BIT | TYPE_C_OR_U_USB_BIT,TYPE_C_OR_U_USB_BIT);
			if (rc < 0) 
			{
				dev_err(chg->dev, "Couldn't %s micro USB mode rc=%d\n",gpio_get_value(chg->pogo6pin_dcin_det)?"disable":"enable",  rc);
			}
			irq_set_irq_type(chg->pogo6pin_dcin_det_irq,IRQF_TRIGGER_HIGH);
		}
		else
		{
			rc = smblib_masked_write(chg, TYPE_C_CFG_REG, APSD_START_ON_CC_BIT | TYPE_C_OR_U_USB_BIT,APSD_START_ON_CC_BIT);
			if (rc < 0) 
			{
				dev_err(chg->dev, "Couldn't %s micro USB mode rc=%d\n",gpio_get_value(chg->pogo6pin_dcin_det)?"disable":"enable",  rc);
			}
			irq_set_irq_type(chg->pogo6pin_dcin_det_irq,IRQF_TRIGGER_LOW);
		}
	}
	irq_count = !irq_count;
	pr_err("pogopin_work irq_count=%d\n",irq_count);
    enable_irq(chg->pogo6pin_dcin_det_irq);
}
static irqreturn_t pogopin_handler(int irq, void *dev_id)
{
	struct smb_charger *chg = dev_id;
	char *pogo6_insert[2] = { "POGO_STATE=POGO6PIN_INSERT", NULL };
	char *pogo6_remove[2] = { "POGO_STATE=POGO6PIN_REMOVE", NULL };

	if(irq == chg->pogo6pin_detect_id_irq) {
		if(chg->last_pogo6pin_isinsert != ! gpio_get_value(chg->pogo6pin_detect_id)) {
			chg->last_pogo6pin_isinsert =  ! gpio_get_value(chg->pogo6pin_detect_id);
			if(!chg->last_pogo6pin_isinsert)
				chg->pogo6pin_remove_count++;
			pr_err("pogo irq: %d,pogo 6pin is %s, count %d\n",  irq, chg->last_pogo6pin_isinsert?"insert":"remove", chg->pogo6pin_remove_count); 
			kobject_uevent_env(&chg->dev->kobj,
					KOBJ_CHANGE, chg->last_pogo6pin_isinsert?pogo6_insert:pogo6_remove);
			if(gpio_is_valid(chg->pogo6pin_5v_out)) {
				pr_err("pogo irq: %d,set 6pin 5v_out to %d \n",  irq, chg->last_pogo6pin_isinsert);
				gpio_direction_output (chg->pogo6pin_5v_out,  chg->last_pogo6pin_isinsert);
			}
		}
	}else if(irq ==  chg->pogo6pin_dcin_det_irq){
        disable_irq_nosync(chg->pogo6pin_dcin_det_irq);
		if(!irq_count)
	    {
			pr_err("[meig]pogo6pin_dcin_det_irq irq_count=%d\n",irq_count);
			return IRQ_HANDLED;
	    }
	   irq_count = !irq_count;
	   pr_err("[meig]IRQ: %s irq_count=%d\n", irq_count);
	   schedule_delayed_work(&chg->pogo_pin_work,msecs_to_jiffies(120));
	}

	return IRQ_HANDLED;
}

static irqreturn_t finger_interrupt(int irq, void *dev_id)
{
	struct smb_charger *chg = dev_id;
	disable_irq_wake(chg->pogo8pin_fb_int_irq);

	if (gpio_get_value(chg->pogo8pin_fb_int) == 0) {
		input_report_key(chg->input_dev, KEY_F6, 1);
		input_report_key(chg->input_dev, KEY_F6, 0);
		input_sync(chg->input_dev);
	...
	enable_irq_wake(chg->pogo8pin_fb_int_irq);
	return IRQ_HANDLED;
}

static int input_device_init(void *dev_id)
{
	int rc = 0;
	struct smb_charger *chg = dev_id;
	chg->input_dev = input_allocate_device();
	if (chg->input_dev == NULL) {
		pr_err("Failed to allocate Input device for hall test!\n");
		return -ENOMEM;
	}

	input_set_capability(chg->input_dev, EV_KEY,
			     KEY_F6); //linux,code = <64>;

	chg->input_dev->name = "finger_input";
	rc = input_register_device(chg->input_dev);
	if (rc) {
		pr_err("Failed register %s input device\n",
		       chg->input_dev->name);
	}
...
static int pogo_pin_parse_dt(struct smb2 *chip)
{
	struct smb_charger *chg = &chip->chg;
	struct device_node *node = chg->dev->of_node;
	int ret = 0;
	u32 flag;
	INIT_DELAYED_WORK(&chg->pogo_pin_work, pogopin_work);//注册工作队列
	chg->pogo6pin_5v_out = of_get_named_gpio_flags(node, "qcom,pogo6pin-5v-out", 0, &flag);
	if (!gpio_is_valid(chg->pogo6pin_5v_out)) {
		pr_err("pogo6pin_5v_out: %d\n", chg->pogo6pin_5v_out);
		ret =  -EINVAL;
		goto err_out;
	}
	ret = gpio_request(chg->pogo6pin_5v_out, "POGO_5V_OUT");
	if (ret < 0) {
		pr_err("Request pogo6pin_5v_out GPIO failed, ret = %d\n", ret);
		goto err_out;
	}
	gpio_direction_output (chg->pogo6pin_5v_out, 0);

	chg->pogo6pin_detect_id = of_get_named_gpio_flags(node, "qcom,pogo6pin-detect-id", 0, &flag);
	if (!gpio_is_valid(chg->pogo6pin_detect_id)) {
		pr_err("qcom,pogo6pin-detect-id: %d\n", chg->pogo6pin_detect_id);
		ret =  -EINVAL;
		goto err_out;
	}
	ret = gpio_request(chg->pogo6pin_detect_id, "POGO_INT");
	if (ret < 0) {
		pr_err("Request IRQ GPIO failed, ret = %d\n", ret);
		goto err_out;
	}
	gpio_direction_input(chg->pogo6pin_detect_id);
	chg->pogo6pin_detect_id_irq = gpio_to_irq(chg->pogo6pin_detect_id);
	ret = devm_request_threaded_irq(chg->dev, chg->pogo6pin_detect_id_irq , NULL,
			pogopin_handler,
			IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING  | IRQF_ONESHOT,
			"pogo_stat_irq", chg);
	if (ret) {
		pr_err("Failed STAT irq=%d request rc = %d\n",
			chg->pogo6pin_detect_id_irq,  ret);
		goto err_out;
	}
	chg->pogo6pin_dcin_det = of_get_named_gpio_flags(node, "qcom,pogo6pin-dcin-det", 0, &flag);
	ret = gpio_request(chg->pogo6pin_dcin_det, "POGODC_INT");
	if (ret < 0) {
		pr_err("Request pogo6pin_dcin_det GPIO failed, ret = %d\n", ret);
		goto err_out;
	}
	gpio_direction_input(chg->pogo6pin_dcin_det);
	#if 0
	chg->pogo6pin_dcin_det_irq = gpio_to_irq(chg->pogo6pin_dcin_det);
	ret = devm_request_threaded_irq(chg->dev, chg->pogo6pin_dcin_det_irq , NULL,
			pogopin_handler,
	...
	#endif
	INIT_DELAYED_WORK(&chg->pogo_dcin_work, pogodcin_work);
	
	chg->pogo8pin_5v_out = of_get_named_gpio_flags(node, "qcom,pogo8pin-5v-out", 0, &flag);
	if (!gpio_is_valid(chg->pogo8pin_5v_out)) {
		pr_err("pogo8pin_5v_out: %d\n", chg->pogo8pin_5v_out);
		ret =  -EINVAL;
		goto err_out;
	}
	ret = gpio_request(chg->pogo8pin_5v_out, "POGO8_5V_OUT");
	if (ret < 0) {
		pr_err("Request pogo8pin_5v_out GPIO failed, ret = %d\n", ret);
		goto err_out;
	}
	gpio_direction_output (chg->pogo8pin_5v_out, 0);

	chg->pogo8pin_fb_int = of_get_named_gpio_flags(node, "qcom,pogo8pin-fb-int", 0, &flag);
	ret = gpio_request(chg->pogo8pin_fb_int, "POGOFB_INT");
	if (ret < 0) {
		pr_err("Request pogo8pin_fb_int GPIO failed, ret = %d\n", ret);
		goto err_out;
	}
	input_device_init(chg);
	gpio_direction_input(chg->pogo8pin_fb_int);
	chg->pogo8pin_fb_int_irq = gpio_to_irq(chg->pogo8pin_fb_int);
	ret = devm_request_threaded_irq(chg->dev, chg->pogo8pin_fb_int_irq , NULL,
			finger_interrupt,
			IRQF_TRIGGER_RISING | IRQF_ONESHOT,
			"finger_irq", chg);
	if (ret) {
		pr_err("Failed pogo8pin_fb_int irq=%d request rc = %d\n",
			chg->pogo8pin_fb_int_irq,  ret);
		goto err_out;
	}
	enable_irq_wake(chg->pogo8pin_fb_int_irq);
	chg->ovp_usb_en = of_get_named_gpio_flags(node, "qcom,ovp-usb-en", 0, &flag);
	if (!gpio_is_valid(chg->ovp_usb_en)) {
		pr_err("ovp_usb_en: %d\n", chg->ovp_usb_en);
		ret =  -EINVAL;
		goto err_out;
	}
	ret = gpio_request(chg->ovp_usb_en, "OVP_USB_EN");
	if (ret < 0) {
		pr_err("Request ovp_usb_en GPIO failed, ret = %d\n", ret);
		goto err_out;
	}
	gpio_direction_output (chg->ovp_usb_en, 0);

	pogopin_handler(chg->pogo6pin_detect_id_irq, chg);
	pogopin_handler(chg->pogo8pin_fb_int_irq, chg);
	schedule_delayed_work(&chg->pogo_dcin_work, msecs_to_jiffies(500)); 
	pr_err("#######pogo6pin %d %d %d %d %d\n", chg->pogo6pin_detect_id,  chg->pogo6pin_dcin_det, chg->pogo6pin_5v_out, chg->pogo8pin_fb_int, chg->pogo8pin_5v_out);
	return 0;
err_out:
	if(gpio_is_valid(chg->pogo6pin_detect_id)){
		gpio_free(chg->pogo6pin_detect_id);
		chg->pogo6pin_detect_id_irq = -EINVAL;
		chg->pogo6pin_detect_id = -EINVAL;
	}
	if(gpio_is_valid(chg->pogo6pin_dcin_det)){
		gpio_free(chg->pogo6pin_dcin_det);
		chg->pogo6pin_dcin_det_irq = -EINVAL;
		chg->pogo6pin_dcin_det = -EINVAL;
	}
	if(gpio_is_valid(chg->pogo6pin_5v_out)){
		gpio_free(chg->pogo6pin_5v_out);
		chg->pogo6pin_5v_out  = -EINVAL;
	}
	if(gpio_is_valid(chg->pogo8pin_fb_int)){
		gpio_free(chg->pogo8pin_fb_int);
		chg->pogo8pin_fb_int_irq  = -EINVAL;
		chg->pogo8pin_fb_int  = -EINVAL;
	}
	if(gpio_is_valid(chg->pogo8pin_5v_out)){
		gpio_free(chg->pogo8pin_5v_out);
		chg->pogo8pin_5v_out = -EINVAL;
	}
	if(gpio_is_valid(chg->ovp_usb_en)){
		gpio_free(chg->ovp_usb_en);
		chg->ovp_usb_en = -EINVAL;
	}
	return ret;
}
// add  by meiglink for task34189 20230426 end ––>
...
static int smb2_parse_dt(struct smb2 *chip)
{
	...
	chg->ufp_only_mode = of_property_read_bool(node,
					"qcom,ufp-only-mode");
	//<!–– add by meiglink for task 35273  20230510 begin
	rc = of_property_read_string(node, "qcom,ext-psy-name",
					&chg->ext_fg_psy_name);
	if (rc)
		chg->ext_fg_psy_name = NULL;
	else
		pr_err("get ext_fg_psy_name %s\n", chg->ext_fg_psy_name);
	// add  by meiglink for  task 35273  20230510 end ––>
	//<!–– add by meiglink for task 37786  20230612 begin
	chg->otg_5v_en = of_get_named_gpio_flags(node, "qcom,otg-5v-en", 0, &flag);
	if (gpio_is_valid(chg->otg_5v_en)) {
		rc = gpio_request(chg->otg_5v_en, "OTG_5V_EN");
		if (rc < 0) {
			pr_err("Request otg_5v_en GPIO failed, rc = %d\n", rc);
			chg->otg_5v_en  = -EINVAL;
		}else{
			gpio_direction_output (chg->otg_5v_en, 0);
		}
	}else{
		pr_err("otg 5v en is not specift\n");
	}
	// add  by meiglink for  task 37786  20230612 end ––>
	pogo_pin_parse_dt(chip);
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TYPEC_MODE:
		...
		if(chg->fake_typec_mode != POWER_SUPPLY_TYPEC_NONE)
			val->intval = chg->fake_typec_mode;
		break;
		...
		const union power_supply_propval *val)
{
	...
	if (!chg->typec_present) {
		...
		case POWER_SUPPLY_PROP_MOISTURE_DETECTED:
			...
			break;
		case POWER_SUPPLY_PROP_TYPEC_MODE:
			if(val->intval != chg->fake_typec_mode){
				chg->fake_typec_mode = val->intval;
				pr_err("@@@@set typec mode %d\n",  chg->fake_typec_mode);
...
//<!–– add by meiglink for task 35273  20230511 begin
	if (chg->ext_fg_psy) {
		switch(psp){
			case POWER_SUPPLY_PROP_TEMP:
				if(chg->fake_battery_temp){
					val->intval = chg->fake_battery_temp;
					goto out;
				}else{
					rc = smblib_get_prop_from_bq27542(chg, psp, val);
					goto out;
				}
	...
	// add  by meiglink for  task 35273  20230511 end ––>
	switch (psp) {
	case POWER_SUPPLY_PROP_STATUS:
		rc = smblib_get_prop_batt_status(chg, val);
		if( val->intval == POWER_SUPPLY_STATUS_DISCHARGING){
			elohub_hardreset_cnt = 0;
		}
		break;
	...
	case POWER_SUPPLY_PROP_TEMP:
		val->intval = 25;
		break;
	//<!–– add by meiglink for task 40884  20230808 begin
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		val->intval = 0;
		break;
	// add  by meiglink for  task 40884  20230808 end ––>
...
//meig_add
static void set_batt_charging_enable(struct smb_charger *chg,bool charging_enable)
{
	int rc;
	if(charging_enable==false)
	{
	    pr_err("MEIG not charging\n");
	...
	{
		 pr_err("MEIG start charging\n");
	    rc = smblib_masked_write(chg, CHARGING_ENABLE_CMD_REG,CHARGING_ENABLE_CMD_BIT,1);
		vote(chg->pl_disable_votable, USER_VOTER, false, 0);
	}
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_DIE_HEALTH:
		...
		break;
	case POWER_SUPPLY_PROP_BATTERY_CHARGING_ENABLED:
		set_batt_charging_enable(chg,!val->intval);
		break;
	case POWER_SUPPLY_PROP_TEMP:
...
static int smb2_init_hw(struct smb2 *chip)
{
	...
	if (rc < 0) {
	 ...
	 //<!–– add by meiglink for task35344  20230522 begin
	if(chg->ext_fg_psy){
		union power_supply_propval pval = {0, };
		rc = smblib_get_prop_from_bq27542(chg,  POWER_SUPPLY_PROP_PRESENT, &pval);
		pr_err("@@@@battery present rc %d %d\n",  rc, pval.intval);
		if(!rc && !pval.intval){
			pr_err("@@@@battery is miss\n");
			chg->battery_miss = true;
			__debug_mask = 0;
			//chip->dt.hvdcp_disable = true;
			//chip->dt.no_pd = true;
		}else{
			chg->battery_miss = false;
		}
	}
	if((chg->pogo6pin_dcin_det_irq > 0) &&!chg->battery_miss)
		pogopin_handler(chg->pogo6pin_dcin_det_irq, chg);
	// add  by meiglink for  task35344  20230522 end ––>
	...
	rc = smblib_read(chg, USBIN_AICL_OPTIONS_CFG_REG, &r_stat);
    pr_err("%s %s %d USBIN_AICL_OPTIONS_CFG_REG=0x%02x\n\r",__FILE__,__func__,__LINE__,r_stat);
	if (rc < 0) {
		pr_err("Couldn't read USBIN_AICL_OPTIONS_CFG_REG rc=%d\n", rc);
	}
	...
	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure THERMREG rc=%d\n", rc);
	}

    //disable SCHG_CHGR_SAFETY_TIMER_ENABLE_CFG regisger
	rc = smblib_masked_write(chg, 0x10a0,
			BIT(0)
				|BIT(1), 0);

	if (rc < 0) {
		dev_err(chg->dev, "Couldn't configure SAFE_REG rc=%d\n", rc);
	}
	...
	//<!–– add by meiglink for task35344  20230524 begin
	if(chg->battery_miss &&  (chg->pogo6pin_dcin_det_irq > 0) &&!gpio_get_value(chg->pogo6pin_dcin_det)){
		dev_err(chg->dev, "@@@@@@battery miss & dc is present, not config typec\n");
		goto dis_config;
	}
	// add  by meiglink for task35344  20230524 end ––>
	//meig_add
	...
	//<!–– add by meiglink for task 35344  20230714 begin
	// if(chg->battery_miss){
	// 	dev_err(chg->dev, "battery miss config typec RD\n");
	// 	rc = smblib_masked_write(chg, RID_CC_CONTROL_15_8_REG,
	// 			 ENABLE_RD_CC_1_BIT, ENABLE_RD_CC_1_BIT);
	// 	if (rc < 0) {
	// 		dev_err(chg->dev, "Couldn't configure ENABLE_CRUDESEN_CC_0_BIT rc=%d\n", rc);
	// 		return rc;
	// 	}
	// 			rc = smblib_masked_write(chg, RID_CC_CONTROL_7_0_REG,
	// 			 ENABLE_RD_CC_0_BIT, ENABLE_RD_CC_0_BIT);
	// 	if (rc < 0) {
	// 		dev_err(chg->dev, "Couldn't configure ENABLE_CRUDESEN_CC_0_BIT rc=%d\n", rc);
	// 		return rc;
	// 	}
	// }
	// add  by meiglink for  task 35344  20230714 end ––>
dis_config:
...
static int smb2_probe(struct platform_device *pdev)
{
    ...
    #ifdef CONFIG_RECHARGE
    chg->bat_recharge_flag = 0;
    chg->last_typec_mode = 0;
    #endif
