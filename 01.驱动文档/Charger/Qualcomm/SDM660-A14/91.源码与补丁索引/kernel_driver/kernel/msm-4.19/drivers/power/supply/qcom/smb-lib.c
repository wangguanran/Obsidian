...
static const struct apsd_result *smblib_get_apsd_result(struct smb_charger *chg)
{
	...
	//meig_add
	if((stat==0x01)||(stat==0x04))//sdp-enumeration-done
	{
		pr_err("[meig]sdp mode\n");
		apsd_type=true;
	}
	else
	{
		pr_err("[meig]dcp mode\n");
		apsd_type=false;
	}
	//meig_end
	...
	if(gpio_is_valid(chg->pogo6pin_dcin_det)&& !gpio_get_value(chg->pogo6pin_dcin_det)){
		//smblib_err(chg, "pogo6pin_dcin_det  set apsd FLOAT\n");
		...
		unsigned long ev, void *v)
{
	...
	if(chg->ext_fg_psy_name&&!strcmp(psy->desc->name, chg->ext_fg_psy_name)){
		if (!chg->ext_fg_psy)
			chg->ext_fg_psy = psy;
			if (ev == PSY_EVENT_PROP_CHANGED)
				schedule_work(&chg->bms_update_work);
...
#define DC_IN_BATT_MISS_MIN_MA 3000
	if(chg->battery_miss ){
		pr_err("@@@@@@@smblib_set_icl_current %d, battery miss %d\n", icl_ua, chg->battery_miss);
		if(icl_ua <= USBIN_25MA ){
			return rc;
		}else if(icl_ua < DC_IN_BATT_MISS_MIN_MA && (chg->pogo6pin_dcin_det_irq > 0) &&!gpio_get_value(chg->pogo6pin_dcin_det)){
			icl_ua = DC_IN_BATT_MISS_MIN_MA;
		}
	}
...
static int _smblib_vbus_regulator_enable(struct regulator_dev *rdev)
{
	...
	if (gpio_is_valid(chg->otg_5v_en)) {
		gpio_direction_output (chg->otg_5v_en, 1);
	}
...
int bq27542_get_prop_batt_status(struct smb_charger *chg,
				union power_supply_propval *val)
{
	union power_supply_propval pval = {0, };
	...
	if (chg->ext_fg_psy) {
		rc = power_supply_get_property(chg->ext_fg_psy, POWER_SUPPLY_PROP_CAPACITY, &pval);
		if (rc < 0) {
			smblib_err(chg, "Couldn't get ext_fg POWER_SUPPLY_PROP_capacity property rc=%d\n", rc);
			return rc;
		}
		soc = pval.intval;

		rc = power_supply_get_property(chg->ext_fg_psy, POWER_SUPPLY_PROP_STATUS, &pval);
		if (rc < 0) {
			smblib_err(chg, "Couldn't get ext_fg POWER_SUPPLY_PROP_status property rc=%d\n", rc);
			return rc;
		}
		status = pval.intval;
	}
	...
	smblib_read(chg, CHARGING_ENABLE_CMD_REG, &stat);
	pr_debug("chg_en reg = %d\n", stat);

	if (!usb_online && !dc_online)
		val->intval =  POWER_SUPPLY_STATUS_DISCHARGING;
	else if ((soc == 100) && (status == POWER_SUPPLY_STATUS_FULL))
		val->intval =  POWER_SUPPLY_STATUS_FULL;
	else if (stat == 1)
		val->intval =  POWER_SUPPLY_STATUS_CHARGING;
	else
		val->intval =  POWER_SUPPLY_STATUS_NOT_CHARGING;
	pr_debug("%s: get status = %d\n", __func__, val->intval);
				...
				union power_supply_propval *val)
{
   ...
   if(gpio_get_value(HW1_GPIO12)==0)
   {
...
{
	int rc, settled_icl_ua, project = 0;
	u8 stat;
...
static int get_rp_based_dcp_current(struct smb_charger *chg, int typec_mode)
{
	...
	/* fall through */
	default:
		//<!–– add by meiglink for task 35344  20230712 begin
		rp_ua = chg->dcp_icl_ua > 0? chg->dcp_icl_ua:DCP_CURRENT_UA;
		// add  by meiglink for  task 35344  20230712 end ––>
	}
...
static void smblib_force_legacy_icl(struct smb_charger *chg, int pst)
{
	...
	case POWER_SUPPLY_TYPE_USB_FLOAT:
		 ...
		 */
		vote(chg->usb_icl_votable, LEGACY_UNKNOWN_VOTER, true, chg->dcp_icl_ua >0?chg->dcp_icl_ua:100000);
		break;
	case POWER_SUPPLY_TYPE_USB_HVDCP:
	case POWER_SUPPLY_TYPE_USB_HVDCP_3:
...
int smblib_init(struct smb_charger *chg)
{
	...
	case PARALLEL_MASTER:
		...
		//<!–– add by meiglink for task 35273  20230510 begin
		if(chg->ext_fg_psy_name)
			chg->ext_fg_psy = power_supply_get_by_name(chg->ext_fg_psy_name);
		if(chg->ext_fg_psy)
			pr_err("@@@@get ext_fg_psy\n");
		else
			pr_err("@@@@not get ext_fg_psy\n");
		// add  by meiglink for  task 35273  20230510 end ––>
