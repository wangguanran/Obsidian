...
/* Modify:wangguanran on: Sun, 04 Jan 2026 03:25:14 +0000 */
...
static void smblib_notify_usb_host(struct smb_charger *chg, bool enable)
{
	...
	pr_info("%s: smb5 sending EXTCON_USB_HOST=%d, extcon=%p, name=%s\n",
		__func__, enable, chg->extcon, extcon_get_edev_name(chg->extcon));
...
int smblib_get_prop_from_bq27542(struct smb_charger *chg,
				...
				union power_supply_propval *val)
{
	...
	rc = power_supply_get_property(chg->ext_fg_psy, psp, val);
	return rc;
}
...
void smblib_rerun_apsd(struct smb_charger *chg)
{
	...
	if(chg->battery_miss){
		pr_err("battery is miss, donot rerun apsd\n");
		return;
	}
...
int smblib_set_icl_current(struct smb_charger *chg, int icl_ua)
{
	...
	if(chg->battery_miss ){
		pr_err("smblib_set_icl_current %d, battery miss %d\n", icl_ua, chg->battery_miss);
...
{
	struct smb_charger *chg = rdev_get_drvdata(rdev);
/* Modify:wangguanran on: Sun, 04 Jan 2026 03:27:32 +0000
	int rc;
  ...
  */
	pr_info("[POGO_USB]%s:%d enabling OTG\n", __func__, __LINE__);
	gpio_direction_output(chg->pogo_ovp_typec_gpio, 1);
	msleep(20);
	gpio_direction_output(chg->pogo_en_typec_5v_gpio, 1);
// End of Modify:    * wangguanran*
	return 0;
}
...
{
	struct smb_charger *chg = rdev_get_drvdata(rdev);
/* Modify:wangguanran on: Sun, 04 Jan 2026 03:28:29 +0000
 * replace this line *
	int rc;
  ...
  */
	pr_info("[POGO_USB]%s:%d disabling OTG\n", __func__, __LINE__);
	gpio_direction_output(chg->pogo_en_typec_5v_gpio, 0);
	msleep(20);
	gpio_direction_output(chg->pogo_ovp_typec_gpio, 0);
// End of Modify:    * wangguanran*
	return 0;
}
				...
				union power_supply_propval *val)
{
	...
	int bat_capacity_val = 0;
	/*start recharge set 95%*/
	union power_supply_propval capacity_val = {0, };

	if (chg->ext_fg_psy) {
		rc = power_supply_get_property(chg->ext_fg_psy,
			POWER_SUPPLY_PROP_CAPACITY, &capacity_val);
	}
	bat_capacity_val = capacity_val.intval;
	/*end recharge set 95%*/
#endif
...
#ifdef CONFIG_RECHARGE
    /*start recharge set 95%*/
    if((val->intval == POWER_SUPPLY_STATUS_FULL) && (bat_capacity_val == 100))
	{
		chg->bat_recharge_flag = 1;
	}
	if((val->intval == POWER_SUPPLY_STATUS_FULL) && (chg->bat_recharge_flag)){
		if(bat_capacity_val < 95) {
			...
			pr_err("MeiG:CHG Battery is below 95,enable recharging! \n");
		}
	}
    /*end recharge set 95%*/
#endif
					  ...
					  union power_supply_propval *val)
{
	...
	/*
	 * Report charger capability for healthd / SystemUI fast-charge icon.
	 * Do NOT use ICL_STATUS (AICL settle): it fluctuates (e.g. 0.75A/0.9A)
	 * and crosses the 7.5W threshold with 9.4V, causing icon flicker.
	 *
	 * PD/PPS: PD_VOTER (negotiated PDO current)
	 * BC1.2:  SW_ICL_MAX_VOTER (APSD/Type-C configured ICL)
	 * else:   USBIN_CURRENT_LIMIT_CFG (programmed ICL)
	 */
	if (chg->pd_active) {
					...
					int usb_current)
{
	...
	if (chg->real_charger_type == QTI_POWER_SUPPLY_TYPE_USB_FLOAT) {
		if (usb_current == -ETIMEDOUT) {
			 ...
			 * real_charger_type
			 */
/* Modify:wangguanran on: Sun, 04 Jan 2026 03:29:28 +0000
			chg->real_charger_type = POWER_SUPPLY_TYPE_USB;
			...
			if (rc < 0)
				return rc;
  */
            if (gpio_get_value(chg->pogo_irq_typec_det_gpio) == 0)
            {
                chg->real_charger_type = QTI_POWER_SUPPLY_TYPE_USB_FLOAT;
                rc = vote(chg->usb_icl_votable, USB_PSY_VOTER, true, 2000000);
                pr_err("[POGO_USB]%s:%d [chg xx] dc charger icl set 2000ma \n", __func__, __LINE__);
            }
            else
            {
                chg->real_charger_type = POWER_SUPPLY_TYPE_USB;
...
// End of Modify:            * wangguanran*
		...
		/*
		 * For dedicated chargers (DCP/CDP/HVDCP), do not disable
		 * SW_ICL_MAX_VOTER. Let the charger type detection logic control
		 * the ICL. Only apply USB_PSY_VOTER override for SDP/FLOAT types.
		 *
		 * This prevents the upper USB framework from overriding the
		 * hardware-detected charger current limit (1.5A for DCP, etc.)
		 * with the default SDP current (500mA).
		 */
		 switch (chg->real_charger_type) {
			...
			case QTI_POWER_SUPPLY_TYPE_USB_HVDCP_3:
			case QTI_POWER_SUPPLY_TYPE_USB_HVDCP_3P5:
				smblib_err(chg, "[FLOW] handle_usb_current: Dedicated charger detected (type=%d), "
					"skip USB_PSY_VOTER override, preserve SW_ICL_MAX_VOTER from charger type\n",
					chg->real_charger_type);
				return 0;
			default:
				/* For SDP, UNKNOWN, FLOAT, etc., use original logic */
				smblib_err(chg, "[FLOW] handle_usb_current: Non-dedicated charger (type=%d), "
					"applying USB_PSY_VOTER override\n",
					chg->real_charger_type);
				break;
		}
...
static void update_sw_icl_max(struct smb_charger *chg, int val)
{
	...
	case QTI_POWER_SUPPLY_TYPE_USB_FLOAT:
		 ...
		 */
		vote(chg->usb_icl_votable, SW_ICL_MAX_VOTER, true,
/* Modify:wangguanran on: Sun, 04 Jan 2026 03:30:21 +0000
					SDP_100_MA);
  */
					2000000);
// End of Modify:            * wangguanran*
...
int smb5_typec_re_attach(struct smb_charger *chg)
{
	u8 stat = 0;
	bool attached = false;
	int rc = 0;
	pr_err("[debug]%s,start!!\n",__func__);
	if(chg->battery_miss){
		pr_err("@@@@@@battery miss, skip typec_re_attach\n");
		goto retn;
	}
	...
	if (!attached) {
		pr_err("[debug]%s,can not attach any device!!\n",__func__);
		goto retn;
	} else {
		/* Step 1: usb_mode = 1 (detach) - triggers detach interrupt */
		rc = smblib_masked_write(chg, TYPEC_U_USB_CFG_REG,
			EN_MICRO_USB_MODE_BIT, EN_MICRO_USB_MODE_BIT);
		pr_err("[usb_mode] writing TYPEC_U_USB_CFG_REG=1, rc=%d\n", rc);
		if (rc < 0) {
			pr_err("[usb_mode] Error writing TYPEC_U_USB_CFG_REG=1, rc=%d\n", rc);
			return rc;
		}

		/* wait 200ms for detach interrupt processing */
		usleep_range(200000, 210000);

		/* Step 2: usb_mode = 0 (attach) - triggers attach interrupt */
		rc = smblib_masked_write(chg, TYPEC_U_USB_CFG_REG,
			EN_MICRO_USB_MODE_BIT, 0);
		pr_err("[usb_mode] writing TYPEC_U_USB_CFG_REG=0, rc=%d\n", rc);
		if (rc < 0) {
			pr_err("[usb_mode] Error writing TYPEC_U_USB_CFG_REG=0, rc=%d\n", rc);
			/* 回滚：第一步 detach 已成功，尝试恢复 EN_MICRO_USB_MODE_BIT=0 (attach 状态) */
			smblib_masked_write(chg, TYPEC_U_USB_CFG_REG,
				EN_MICRO_USB_MODE_BIT, 0);
			return rc;
		}
	}
...
static void smblib_typec_reattach_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger,
	                                       typec_reattach_work.work);
	int rc = 0;
	pr_err("Enter smblib_typec_reattach_work, real_charger_type:%d\n", chg->real_charger_type);
	rc = smb5_typec_re_attach(chg);
	pr_err("[debug]%s,rc =%d\n",__func__,rc);
...
#ifdef CONFIG_RECHARGE
	pr_err("MeiG:CHG func:%s ,last_typec_mode = %d , typec_mode = %d\n",__func__, chg->last_typec_mode, chg->typec_mode);
	if(((!chg->last_typec_mode) ||(!chg->typec_mode)))
			chg->bat_recharge_flag = 0;
	chg->last_typec_mode = chg->typec_mode;
#endif
...
static bool smblib_pogo_dc_dock_present(struct smb_charger *chg)
{
	if (!gpio_is_valid(chg->pogo_irq_back_9v_det_gpio))
		return false;

	/* GPIO low = POGO DC charge dock present */
	return gpio_get_value(chg->pogo_irq_back_9v_det_gpio) == 0;
}

static void smblib_pogo_apply_fixed_dc_icl(struct smb_charger *chg)
{
	int rc;
	...
	mutex_lock(&chg->dcin_aicl_lock);
	rc = smblib_set_charge_param(chg, &chg->param.dc_icl, POGO_DC_ICL_UA);
	mutex_unlock(&chg->dcin_aicl_lock);
	if (rc < 0)
		smblib_err(chg, "POGO DC: set dc_icl=%duA failed rc=%d\n",
				POGO_DC_ICL_UA, rc);

	chg->dcin_aicl_done = true;
	chg->dcin_icl_user_set = true;
	chg->dcin_uv_count = 0;

	if (chg->fcc_votable)
		vote(chg->fcc_votable, WLS_PL_CHARGING_VOTER, true,
			4000000);
	if (chg->fcc_main_votable)
...
irqreturn_t smb5_dc_plugin_irq_handler(int irq, void *data)
{
	...
	if (dcin_present && !vbus_present) {
		...
		if (smblib_pogo_dc_dock_present(chg)) {
			smblib_pogo_apply_fixed_dc_icl(chg);
			goto dc_plugin_done;
		}
...
/* Modify:wangguanran on: Sun, 04 Jan 2026 03:31:32 +0000 */
		pr_err("[POGO_USB]%s:%d meig dc_plugin_irq_handler dc plugout dcin_en=0 -> hw auto\n", __func__, __LINE__);
...
/* Modify:wangguanran on: Sun, 04 Jan 2026 03:31:48 +0000 */
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->typec_reattach_work, smblib_typec_reattach_work);

/* Modify:wangguanran on: Tue, 30 Dec 2025 14:25:37 +0800 */
	INIT_DELAYED_WORK(&chg->pogo_irq_back_9v_det_delayed_work,
					smblib_pogo_irq_back_9v_det_delayed_work);
	INIT_DELAYED_WORK(&chg->pogo_irq_typec_det_delayed_work,
					smblib_pogo_irq_typec_det_delayed_work);
	INIT_DELAYED_WORK(&chg->pogo_irq_back_det_delayed_work,
					smblib_pogo_irq_back_det_delayed_work);
// End of Modify:    * wangguanran*
	...
	schedule_delayed_work(&chg->typec_reattach_work, msecs_to_jiffies(7000));
	return rc;
}
...
int smblib_deinit(struct smb_charger *chg)
{
	...
	case PARALLEL_MASTER:
		...
		cancel_delayed_work_sync(&chg->role_reversal_check);
		cancel_delayed_work_sync(&chg->pr_swap_detach_work);
		cancel_delayed_work_sync(&chg->typec_reattach_work);
...
/* Modify:wangguanran on: Wed, 24 Dec 2025 07:47:23 +0000 */
irqreturn_t pogo_irq_back_9v_det_irq_handler(int irq, void *data)
{
    struct smb_irq_data *irq_data = data;
    struct smb_charger *chg = irq_data->parent_data;
    int input_present, rc;
...
irqreturn_t pogo_irq_typec_det_irq_handler(int irq, void *data)
{
    struct smb_irq_data *irq_data = data;
    struct smb_charger *chg = irq_data->parent_data;

    // TODO Reserved interface, switching logic already implemented in smblib_vbus_regulator_enable/disable
    pr_info("[POGO_USB]%s:%d gpio=%d\n", __func__, __LINE__, gpio_get_value(chg->pogo_irq_typec_det_gpio));
    // schedule_delayed_work(&chg->pogo_irq_typec_det_delayed_work, msecs_to_jiffies(100));
    return IRQ_HANDLED;
}
irqreturn_t pogo_irq_back_det_irq_handler(int irq, void *data)
{
    struct smb_irq_data *irq_data = data;
    struct smb_charger *chg = irq_data->parent_data;
    int status = 0;
    status = gpio_get_value(chg->pogo_irq_back_det_gpio);
    schedule_delayed_work(&chg->pogo_irq_back_det_delayed_work, msecs_to_jiffies(100));
    pr_info("[POGO_USB]%s:%d gpio=%d\n", __func__, __LINE__, status);
    return IRQ_HANDLED;
}
void smblib_pogo_irq_back_9v_det_delayed_work(struct work_struct *work)
{
    struct smb_charger *chg = container_of(work, struct smb_charger,
                                           pogo_irq_back_9v_det_delayed_work.work);
    int status = 0;
    status = gpio_get_value(chg->pogo_irq_back_9v_det_gpio);
    pr_info("[POGO_USB]%s:%d pogo_irq_back_9v_det_gpio=%d\n", __func__, __LINE__, status);
}

void smblib_pogo_irq_typec_det_delayed_work(struct work_struct *work)
{
    struct smb_charger *chg = container_of(work, struct smb_charger,
                                           pogo_irq_typec_det_delayed_work.work);
    int status = 0;
    status = gpio_get_value(chg->pogo_irq_typec_det_gpio);
    pr_info("[POGO_USB]%s:%d gpio=%d\n", __func__, __LINE__, status);
    
    /* IRQ3 TypeC OTG detect: low=enable EN1, high=disable EN1 */
    if (gpio_is_valid(chg->pogo_en_typec_5v_gpio)) {
        /* low=enable EN1 (output 1), high=disable EN1 (output 0) */
        gpio_direction_output(chg->pogo_ovp_typec_gpio, status ? 0 : 1);
        pr_info("[POGO_USB]%s:%d set pogo_ovp_typec_gpio=%d (status=%d)\n",
                 __func__, __LINE__, status ? 0 : 1, status);
        gpio_direction_output(chg->pogo_en_typec_5v_gpio, status ? 0 : 1);
        pr_info("[POGO_USB]%s:%d set pogo_en_typec_5v_gpio=%d (status=%d)\n",
                 __func__, __LINE__, status ? 0 : 1, status);
    }
    
    if (status == 0)
    {
        vote(chg->usb_icl_votable, USB_PSY_VOTER, true, 2000000);
    }
}
...
{
    struct smb_charger *chg = container_of(work, struct smb_charger,
                                           pogo_irq_back_det_delayed_work.work);
    int status = 0;
    status = gpio_get_value(chg->pogo_irq_back_det_gpio);
    pr_info("[POGO_USB]%s:%d pogo_irq_back_det_gpio=%d\n",
             __func__, __LINE__, status);
    
    /* IRQ2 dock presence detect:
     * dock inserted (low): pull Switch1/2 high, enable EN4;
     * dock removed (high): pull Switch1/2 low, disable EN2/3/4 */
    if (status == 0) {
        /* dock inserted (low): pull Switch1/2 high, enable EN4 */
        pr_info("[POGO_USB]%s:%d dock inserted (gpio low)\n", __func__, __LINE__);
        
        /* Switch1 (pogo_sw_typec_hub_gpio): switch to USB hub */
        if (gpio_is_valid(chg->pogo_sw_typec_hub_gpio)) {
            gpio_direction_output(chg->pogo_sw_typec_hub_gpio, 1);
            pr_info("[POGO_USB]%s:%d set pogo_sw_typec_hub_gpio=1\n", __func__, __LINE__);
        }
        
        /* Switch2 (pogo_sw_uart_usb_gpio): switch to USB hub */
        if (gpio_is_valid(chg->pogo_sw_uart_usb_gpio)) {
            gpio_direction_output(chg->pogo_sw_uart_usb_gpio, 1);
        ...
        if (gpio_is_valid(chg->pogo_en_hub_vcc_gpio)) {
            gpio_direction_output(chg->pogo_en_hub_vcc_gpio, 1);
            pr_info("[POGO_USB]%s:%d set pogo_en_hub_vcc_gpio=1 (EN4 on)\n",
                     __func__, __LINE__);
        }
        ...
        /* dock removed (high): pull Switch1/2 low, disable EN2/3/4 */
        pr_info("[POGO_USB]%s:%d dock removed (gpio high)\n", __func__, __LINE__);
        
        /* Switch1 (pogo_sw_typec_hub_gpio): switch back to TypeC */
        if (gpio_is_valid(chg->pogo_sw_typec_hub_gpio)) {
            gpio_direction_output(chg->pogo_sw_typec_hub_gpio, 0);
            pr_info("[POGO_USB]%s:%d set pogo_sw_typec_hub_gpio=0\n", __func__, __LINE__);
        }
        
        /* Switch2 (pogo_sw_uart_usb_gpio): switch back to UART */
        if (gpio_is_valid(chg->pogo_sw_uart_usb_gpio)) {
            gpio_direction_output(chg->pogo_sw_uart_usb_gpio, 0);
            pr_info("[POGO_USB]%s:%d set pogo_sw_uart_usb_gpio=0\n",
                     __func__, __LINE__);
        }
        
        /* Disable EN2 (pogo_en_typea_5v_gpio) */
        if (gpio_is_valid(chg->pogo_en_typea_5v_gpio)) {
            gpio_direction_output(chg->pogo_en_typea_5v_gpio, 0);
            pr_info("[POGO_USB]%s:%d set pogo_en_typea_5v_gpio=0 (EN2 off)\n",
                     __func__, __LINE__);
        }
        
        /* Disable EN3 (pogo_en_micro_5v_gpio) */
        if (gpio_is_valid(chg->pogo_en_micro_5v_gpio)) {
            gpio_direction_output(chg->pogo_en_micro_5v_gpio, 0);
            pr_info("[POGO_USB]%s:%d set pogo_en_micro_5v_gpio=0 (EN3 off)\n",
                     __func__, __LINE__);
        }
        
        /* Disable EN4 (pogo_en_hub_vcc_gpio) */
        if (gpio_is_valid(chg->pogo_en_hub_vcc_gpio)) {
            gpio_direction_output(chg->pogo_en_hub_vcc_gpio, 0);
