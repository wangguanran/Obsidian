# MT5825 NFC VEN 双路径 GPIO51 控制（禅道 Task 118851）

## 基本信息

| 字段 | 值 |
|:---|:---|
| 禅道编号 | Task #118851（非 Bug，设计任务） |
| 问题编号 | ISSUE-2026-0812-002 |
| 项目 | MT5825-11 寸支付平板（禅道 project 903） |
...
| 项 | 值 |
|:---|:---|
| 源码仓 | `/home/wangguanran/MT5825/LA.VENDOR.13.2.1`（改码源仓） |
| 编译仓 | `/home3/wangguanran/codes/MT5825/LA.VENDOR.13.2.1`（含完整 kernel_platform/out） |
...
| 附件 | `/home/wangguanran/aosp-log-analysis-workspace/attachments/TASK-118851/`（236229/236230 原理图） |
...
```
[MT5825][TaskID]118851[Description] add platform GPIO51 NFC VEN control
[Solution] dual-path VEN expander+tlmm51 ven2-first [Owner]wangguanran
```

Change-Id: `I98f217c6fc8e9f0e51ecde112f73a2b25017da87`
...
```bash
# DTS/DTBO（codes 树）
cd /home3/wangguanran/codes/MT5825
JOBS=64 TMPDIR=/home3/wangguanran/tmp ./make_dtbo.sh
...
Patch：`aosp-log-analysis-workspace/patches/禅道#118851/0002-vendor-nfc-ven2-probe-fix-coldboot.patch`
...
| 产物路径 | `/home/wangguanran/MT5825/out/vendor_boot.img`、`out/dtbo.img`、`out/boot.img` |
| job | `MT5825-20260813-014813-256280` |
...
> 原始补丁路径：`/home3/wangguanran/documents/aosp-log-analysis-workspace/patches/禅道#118851/0001-vendor-nfc-ven2-dual-path-gpio51.patch`（与 `0001-vendor-c10e22655f0d-nfc-ven2.patch`、`0001-MT5825-TaskID-118851-Description-add-platform-GPIO51.patch` 内容相同，同一 commit）

```diff
From c10e22655f0dfb4d04c8455f226a9bfdbb8eb2cb Mon Sep 17 00:00:00 2001
From: wangguanran <wangguanran@example.com>
Date: Wed, 12 Aug 2026 06:03:09 +0000
Subject: [PATCH] [MT5825][TaskID]118851[Description] add platform GPIO51 NFC
 VEN control [Solution] dual-path VEN expander+tlmm51 ven2-first
 [Owner]wangguanran
