...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 0))
	bl->pwm_bl = devm_pwm_get(dev, NULL);
#else
...
static void dp_panel_pwm_unregister(struct dp_panel *dp_panel)
{
	...
	/* pwm acquired via devm_pwm_get(); released by device-managed resources */
	if (bl->pwm_bl && bl->pwm_enabled) {
...
static int dp_panel_set_backlight(struct dp_panel *panel, u32 bl_lvl)
{
	...
	if (rc) {
		DP_ERR("failed to change pwm config, rc=%d\n", rc);
		goto error;
	}
	...
	if (bl_lvl != 0 && !bl->pwm_enabled) {
		...
		if (rc) {
			DP_ERR("failed to enable pwm, rc=%d\n", rc);
			goto error;
		}
...
struct dp_panel *dp_panel_get(struct dp_panel_in *in)
{
	...
	dp_panel->bl_config.brightness_max_level =
		panel->parser->brightness_max_level;
	...
	if (panel->parser->is_edp) {
		...
		if (rc)
			DP_ERR("Failed to register pwm, rc=%d\n", rc);
	}
