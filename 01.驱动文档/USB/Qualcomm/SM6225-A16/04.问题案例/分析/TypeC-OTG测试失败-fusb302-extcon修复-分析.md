...
[项目代号]_A16 (SM6225/khaje) 的 Type-C 口硬件由 FUSB302 做 CC 检测，DWC3 做 USB 控制器。TypeC OTG 要工作必须满足：
...
- 与 [[01.驱动文档/USB/Qualcomm/SM6225-A16/01.原理与架构/TypeC-OTG驱动架构分析.md|TypeC-OTG 驱动架构分析]] 描述的链路一致：FUSB302 检测 CC → extcon → dwc3 role switch；
- khaje 平台其它项目（如 GPIO/[项目代号] 平台目录）共用同一套 khaje dtsi，本改动影响所有 include khaje-idp.dtsi 的板型。
