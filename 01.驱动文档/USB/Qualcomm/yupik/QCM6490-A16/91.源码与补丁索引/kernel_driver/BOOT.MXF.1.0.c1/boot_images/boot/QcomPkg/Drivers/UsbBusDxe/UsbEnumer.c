...
/*=============================================================================
 ...
 --------   ---      ----------------------------------------------------------
 07/30/19   gk       Added support to fix memory Leak Issue
 02/13/19   gk       Fixed UEFI SS OTG enumeration Failure
 09/21/18   gk       Fixed KW errors on boot.xf.3.0 
 07/16/18   gk       Fix issue for Memory Leak when flashing NVMe through Uefi Mass Storage
 07/02/18   kameya   Add support for SuperSpeed-Plus enumeration
...
#include "UsbBus.h"
#ifdef USB_CAMERA_FLAG
/*============================ DO NOT PORT TO TARGETS WITHOUT BUILT IN USB CAMERA =======================================
In uefiplat.cfg, static memory is defined for Windows OS static memory usage:
0x9FF90000, 0x00002000, "USB UCSI Temp",     AddMem, SYS_MEM, SYS_MEM_CAP, Reserv, UNCACHED_UNBUFFERED_XN 

Note: Remaining one not used are reserves:
- Offset 0x00 to 0x40:  Windows O/S UCSI PMIC/MSFT uses  48 bytes
- Offset 0x40 to 0x80:  Windows O/S PMIC and QC UCSI uses 6 bytes

Poipu Compal Specific:
- Offset 0x80 to 0x90:  UEFI UsbBusDxe and Windows OS Camera ACPI uses 4 bytes

For Poipu Compal platform only Usage is for UEFI UsbBusDxe to pass isEnumerated flag for MP0 USB2 Port.
If MP0 USB2 Port has device enumerated, then camera ACPI in Windows assumes that USB camera is 
connected and will not load MIPI Camera driver.
====================== end of DO NOT PORT TO TARGETS WITHOUT BUILT IN USB CAMERA =======================================*/

#include <Library/IoLib.h>
#include <Protocol/EFIPlatformInfo.h>

#define USB_CAMERA_PORT               (0x4)
#define USBCAM_EXIST_ADDR             (0x9FF90080)
#define PLATFORM_INFO_SUBTYPE_PRIMUS  (0x0)
#define PLATFORM_INFO_SUBTYPE_COMPAL  (0x1)


/**
  Reads platform information using PlatformInfo protocol.
  Sets camera exists cookie in USB UCSI reserved memory if platform is compal.
...
**/
VOID UsbSetCameraCookie()
{
  static EFI_PLATFORMINFO_PLATFORM_INFO_TYPE  PlatformInfo = {EFI_PLATFORMINFO_TYPE_UNKNOWN, 0, 0, 0};
  EFI_PLATFORMINFO_PROTOCOL                   *hPlatformInfoProtocol;
  EFI_STATUS                                  Status = EFI_SUCCESS;

  if (PlatformInfo.platform == EFI_PLATFORMINFO_TYPE_UNKNOWN)
  {
    MmioWrite32 (USBCAM_EXIST_ADDR, 0);
    ...
    if EFI_ERROR (Status)
    {
      DEBUG ((EFI_D_ERROR, "UsbSetCameraCookie: Failed to locate PlatformInfo protocol\n"));
      return;
    }
    ...
    if (EFI_ERROR (Status))
    {
      DEBUG ((EFI_D_ERROR, "UsbSetCameraCookie: Failed to get PlatformInfo\n"));
      return;
    }

    // Set camera exists cookie in USB UCSI reserved memory space if platform is compal.
    if ((PlatformInfo.platform == EFI_PLATFORMINFO_TYPE_CLS) && (PlatformInfo.subtype == PLATFORM_INFO_SUBTYPE_COMPAL))
    {
      DEBUG ((EFI_D_ERROR, "UsbSetCameraCookie: Set USB camera exist cookie.\n"));
      MmioWrite32 (USBCAM_EXIST_ADDR, 1);
    }
  }
}
#endif
  ...
  )
{
  ...
  if (UsbIsHubInterface (UsbIf)) {
    ...
    if (UsbBusIsWantedUsbIO (UsbIf->Device->Bus, UsbIf)) {
      ...
      if (UsbGetCurrentTpl () != TPL_CALLBACK){
         DEBUG ((EFI_D_ERROR, "TPL Level is not same (%d)\n", (UINT32)UsbGetCurrentTpl()));
         Status = EFI_INVALID_PARAMETER;
         USB_ASSERT_GOTO ((UsbGetCurrentTpl () == TPL_CALLBACK),ON_EXIT);
    }
  ...
  switch (DevState)
  {
    ...
    { // Perform rest of enumeration till the completion
      ...
      if (!USB_BIT_IS_SET (PortState->PortStatus, USB_PORT_STAT_CONNECTION)) {
        ...
        if(PortState->PortStatus > 1){
          DEBUG ((EFI_D_ERROR, "UsbEnumerateNewDev: No device present at port %d after reset, returning with PortStatus %d\n", Port, PortState->PortStatus));
        goto ON_ERROR;
      }
        // For OTG Cable, if it has Portstatus bit set but doesn't have any info regarding device enumeration. Returning in such cases.
        else {
          gBS->FreePool (PortState);
        ...
        for (Index_c = 0; Index_c < Child->DevDesc->Desc.NumConfigurations && Child->DevDesc->Configs[Index_c]; Index_c++) {
          for (Index_i = 0; Index_i < Child->DevDesc->Configs[Index_c]->Desc.NumInterfaces; Index_i++) {
            if (Child->DevDesc->Configs[Index_c]->Interfaces[Index_i] && Child->DevDesc->Configs[Index_c]->Interfaces[Index_i]->Settings[0]) {
              ...
              DEBUG ((EFI_D_ERROR, "UsbEnumNewDevRootHub: dropping Invalid device on port %d\n", Port));
              InvalidDev = TRUE;
              goto ON_ERROR;
            }
          }
      ...
      //
      if (Child && Child->DevDesc && Child->DevDesc->Configs[0])
      {
...
#ifdef USB_CAMERA_FLAG
      //
      // ========== DO NOT PORT TO TARGETS WITHOUT USB CAMERA ==========
      // Set camera exists cookie in USB UCSI reserved memory space.
      //
      if ((UINT8)HubIf->NumOfPort == USB_CAMERA_PORT)
      {
        UsbSetCameraCookie();
      }
#endif
...
ON_ERROR:
  ...
  //
  if ((IsAudioClass == FALSE) && (InvalidDev == FALSE)) {
  ...
  switch (DevState)
  {
    ...
    { // Perform rest of enumeration till the completion
      ...
      if (USB_BIT_IS_SET (PortState->PortStatus, USB_PORT_STAT_SUPER_SPEED_PLUS)) {
        Child->Speed      = EFI_USB_SPEED_SUPER;
        Child->MaxPacket0 = 512;
        DEBUG (( EFI_D_ERROR, "UsbEnumNewDevHub: SUPER_PLUS on port %d\n", Port));
      } else if (USB_BIT_IS_SET (PortState->PortStatus, USB_PORT_STAT_SUPER_SPEED)) {
        Child->Speed      = EFI_USB_SPEED_SUPER;
        Child->MaxPacket0 = 512;
        DEBUG (( EFI_D_ERROR, "UsbEnumNewDevHub: SUPER on port %d\n", Port));
      } else if (USB_BIT_IS_SET (PortState->PortStatus, USB_PORT_STAT_HIGH_SPEED)) {
        Child->Speed      = EFI_USB_SPEED_HIGH;
        Child->MaxPacket0 = 64;
        DEBUG (( EFI_D_ERROR, "UsbEnumNewDevHub: HIGH on port %d\n", Port));
      } else if (USB_BIT_IS_SET (PortState->PortStatus, USB_PORT_STAT_LOW_SPEED)) {
        Child->Speed      = EFI_USB_SPEED_LOW;
        Child->MaxPacket0 = 8;
        DEBUG (( EFI_D_ERROR, "UsbEnumNewDevHub: LOW on port %d\n", Port));
      } else {
        Child->Speed      = EFI_USB_SPEED_FULL;
        Child->MaxPacket0 = 8;
        DEBUG (( EFI_D_ERROR, "UsbEnumNewDevHub: FULL on port %d\n", Port));
      }
        ...
        for (Index_c = 0; Index_c < Child->DevDesc->Desc.NumConfigurations && Child->DevDesc->Configs[Index_c]; Index_c++) {
          for (Index_i = 0; Index_i < Child->DevDesc->Configs[Index_c]->Desc.NumInterfaces; Index_i++) {
            if (Child->DevDesc->Configs[Index_c]->Interfaces[Index_i] && Child->DevDesc->Configs[Index_c]->Interfaces[Index_i]->Settings[0]) {
              ...
              DEBUG ((EFI_D_ERROR, "UsbEnumNewDevHub: dropping  Invalid device on port %d\n", Port));
              InvalidDev = TRUE;
              goto ON_ERROR;
            }
          }
...
ON_ERROR:
  ...
  /*DEBUG ((EFI_D_ERROR, "UsbEnumNewDevHub: << Remove Device >>"));
  if (Address != Bus->MaxDevices) {
    Bus->Devices[Address] = NULL;
  }
  if (Child != NULL) {
