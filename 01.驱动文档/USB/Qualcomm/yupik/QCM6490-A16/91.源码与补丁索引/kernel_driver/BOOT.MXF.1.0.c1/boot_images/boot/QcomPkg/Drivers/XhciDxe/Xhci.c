...
/*=============================================================================
 ...
 --------   ---      ---------------------------------------------------------
 11/29/19   rg       Changes in XhcClearRootHubPortFeature for suspend-resume compliance isssue.
 11/30/18   gk       Removing Chip level dependency from Xhci driver.
 07/02/18   kameya   Add support for SuperSpeed-Plus enumeration
 04/19/18   amitg    Poipu Multi-Port Support, Vbus fixes
  ...
  )
{
  ...
  #if 0 // Commenting out the chip level dependency from Xhci driver. Can Enable it for debuuging purpose.
  ...
  )
{
  ...
  case EfiUsbPortSuspend:
    ...
    switch ((State & XHC_PORTSC_PS) >> 10) {
      ...
      case 4: // Superspeed port
        State &= ~XHC_PORTSC_PLS;
        XhcWriteOpReg (Xhc, Offset, State);
        break;

      default: // USB 2.0 speed port
        State |= (PLSW_USB2_RESUME_VAL << 5);
        XhcWriteOpReg (Xhc, Offset, State);
        gBS->Stall(20 * 1000); // 20 mS delay defined in xHCI spec, sec 4.15.2.2
    State &= ~XHC_PORTSC_PLS;
    XhcWriteOpReg (Xhc, Offset, State);
    break;
    }
  ...
  )
{
  ...
  else
  {
    ...
    if (PciID == ((RENESAS_DEVICE_ID << 16) | RENESAS_VENDOR_ID))
    {
      Status = RenesasXhciCheckRequestFw(PciIo);
      if (EFI_ERROR (Status))
      {
        Status = EFI_UNSUPPORTED;
        goto ON_EXIT;
