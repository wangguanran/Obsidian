...
#endif /* LINUX_VERSION_CODE >= 6.9.0 */
#endif
			  ...
			  struct _ax_ioctl_command *info)
{
    ...
    if(axdev->udev->bus->busnum != ETH0_USB_BUS_NUM){
        pr_info("[%s-%d] usb bus not %d skip\n",__func__,__LINE__,ETH0_USB_BUS_NUM);
        return 0;
    }
