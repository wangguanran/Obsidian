# MC5616 Sensor 裁剪 SSC vendor 库 — 分析
...
Netrani SSC（Sensors SubSystem）的 POR（Power On Reset）配置脚本 `por.py` 中，`include_sensor_vendor_libs` 列表默认包含了多种传感器 vendor 库。这些库对应的是参考设计平台所支持的传感器型号，但 MC5616 项目的实际 BOM 中仅使用了 STMicroelectronics 的 `sns_sc7a20` 加速度传感器。
...
- [[../MC5616-Sensor裁剪SSC-vendor库|MC5616 Sensor 裁剪 SSC vendor 库 — 主文档]]
