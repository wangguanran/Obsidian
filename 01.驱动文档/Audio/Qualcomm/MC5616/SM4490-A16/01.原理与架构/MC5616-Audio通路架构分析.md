# MC5616 (parrot) Audio LPASS 通路架构分析

> **版本号：v1.0**

## 平台概述

MC5616（基于 parrot/QCM4490）的音频子系统使用 **LPASS（Low Power Audio SubSystem）** 内建 codec（lpass-cdc）。音频通路分为 VA（Voice Activity）与 RX（Playback）两大类：
...
LPASS 内建 codec (lpass-cdc)
   ...
   ├─ RX 通路: RX macro → SWR master → 外置 PA   ← MC5616 未使用，已禁用
...
MC5616 板无对应硬件，上述操作无实际意义，还会增加功耗与启动延迟。
...
- 若后续硬件变更需启用 RX 通路，需反向恢复上述 DTS 和驱动修改（仅影响 MC5616/parrot，不涉及其他 parrot 衍生板型）

## 引用文件索引

- [[01.驱动文档/Audio/Qualcomm/MC5616/SM4490-A16/03.需求实现/MC5616-Audio禁用RX-macro-SWR通路.md|MC5616-Audio禁用RX-macro-SWR通路]]（变更明细与补丁）
