...
struct msm_asoc_mach_data {
	...
	unsigned int vcc_micbias1_gpio;
};
...
static int vcc_enable_micbias(struct snd_soc_dapm_widget *w,
					...
					int event)
{
	...
	dev_info(component->dev, "%s: enter, wname: %s, event: %d\n",
		__func__, w->name, event);

	micbias_gpio = pdata->vcc_micbias1_gpio;

	switch (event) {
	case SND_SOC_DAPM_PRE_PMU:
		dev_info(component->dev, "%s: enable micbias gpio(%d)\n",
			__func__, (micbias_gpio - 336));
		gpio_direction_output(micbias_gpio, 1);
		break;
	case SND_SOC_DAPM_POST_PMU:
		usleep_range(1000, 1100);
		break;
	case SND_SOC_DAPM_POST_PMD:
		dev_info(component->dev, "%s: disable micbias gpio(%d)\n",
			__func__, (micbias_gpio - 336));
		gpio_direction_output(micbias_gpio, 0);
		break;
	};
...
static const struct snd_soc_dapm_widget msm_int_dapm_widgets[] = {
	...
	SND_SOC_DAPM_SUPPLY("VCC MIC BIAS1", SND_SOC_NOPM, 0, 0,
			vcc_enable_micbias,
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	pdata->vcc_micbias1_gpio = of_get_named_gpio(pdev->dev.of_node,
			"vcc-micbias1-gpio", 0);
	if (!gpio_is_valid(pdata->vcc_micbias1_gpio)) {
		dev_err(&pdev->dev, "%s: Invalid micbias1 gpio\n", __func__);
	} else {
		ret = gpio_request(pdata->vcc_micbias1_gpio, "vcc-micbias1");
		if (ret < 0) {
			dev_err(&pdev->dev, "%s: Request micbias1 gpio(%d) failed\n",
				__func__, (pdata->vcc_micbias1_gpio - 336));
		} else {
			gpio_direction_output(pdata->vcc_micbias1_gpio, 0);
			dev_info(&pdev->dev, "%s micbias1 gpio : %d\n",
				__func__, (pdata->vcc_micbias1_gpio - 336));
		}
	}
...
static int msm_asoc_machine_remove(struct platform_device *pdev)
{
	...
	if(pdata->vcc_micbias1_gpio)
		gpio_free(pdata->vcc_micbias1_gpio);
